...
// 中文选词
public class ReviewByCNActivity extends AppCompatActivity {
    // 对错控件
    ImageView yOrN1;
    ImageView yOrN2;
    ImageView yOrN3;
    ...
    ArrayList<Word> updateWords;    // 更新列表
    Word currentWord;               // 当前单词
    Button[] buttons;               // 按钮数组
    int[] buttonRandoms;            // 按钮数组下标
    int count;                      // 当前单词位置
    boolean flag;                   // 是否更新熟练等级
    ...
    Handler msgHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            switch(msg.what) {
                case MESSAGE_ERROR:
                    Toast.makeText(ReviewByCNActivity.this, (String)msg.obj, Toast.LENGTH_SHORT).show();
                    break;
                case REQUEST_FAILED:
                    Toast.makeText(ReviewByCNActivity.this, (String) msg.obj, Toast.LENGTH_SHORT).show();
                    break;
                case REQUEST_OK:
                    try {
                        ...
                        Toast.makeText(ReviewByCNActivity.this, "服务器异常", Toast.LENGTH_SHORT).show();
                        e.printStackTrace();
                    }
                    break;
                case REVIEW_NULL:
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_review_by_cn);
        // 初始化变量
        initView();
        // 设置按钮
        try {
    ...
    public void nextWord()
    {
        ...
        if (count > words.size()) {
            ...
            Global.i.reviewCountCn = words.size() + 1;
            msgHandler.postDelayed(new Runnable() {
                @Override
                public void run() {
                    Intent intent = new Intent(ReviewByCNActivity.this, MainActivity.class);
        ...
        if (count != 1 && flag) {
            updateWords.add(currentWord);                   // 添加到更新列表
        }
    ...
    public void reviewNull(Message msg) throws JSONException {
        ...
        Toast.makeText(ReviewByCNActivity.this, tips + ",快去背几个吧.", Toast.LENGTH_LONG).show();
        msgHandler.postDelayed(new Runnable() {
            @Override
            public void run() {
                finish();
    ...
    public void sendPost() throws Exception {
        ...
        req.put("offset", Global.i.reviewCountCn);
    ...
    {
        yOrN1 = (ImageView) findViewById(R.id.id_imageView_reviewByCn_yesOrNo1);
        yOrN2 = (ImageView) findViewById(R.id.id_imageView_reviewByCn_yesOrNo2);
        yOrN3 = (ImageView) findViewById(R.id.id_imageView_reviewByCn_yesOrNo3);
        yOrN4 = (ImageView) findViewById(R.id.id_imageView_reviewByCn_yesOrNo4);

        button1 = (Button) findViewById(R.id.id_button_reviewByCn_contentButton1);
        button2 = (Button) findViewById(R.id.id_button_reviewByCn_contentButton2);
        button3 = (Button) findViewById(R.id.id_button_reviewByCn_contentButton3);
        button4 = (Button) findViewById(R.id.id_button_reviewByCn_contentButton4);

        currentCount = (TextView) findViewById(R.id.id_textView_reviewByCn_currentCount);
        totalCount = (TextView) findViewById(R.id.id_textView_reviewByCn_totalCount);
        wordTranslation = (TextView) findViewById(R.id.id_textView_reviewByCn_translation);
        ...
        updateWords = new ArrayList<Word>();        // 更新列表
    ...
    public void sendPost2() throws Exception {
        ...
        req1.put("currentReviewCn", Global.i.reviewCountCn);
        Rest2.sendRequest(Config.URL + "SetCurrentReviewCn.api", req1, new Callback() {
            @Override
            public void onFailure(Call call, IOException e) {
                e.printStackTrace();
            }
        ...
        if (updateWords.size() != 0) {
            JSONObject req2 = new JSONObject(Global.i.request.toString());
            JSONArray wordList = new JSONArray();
            for (int i = 0; i < updateWords.size(); i++) {
                Word word =  updateWords.get(i);
                JSONObject wordJs = new JSONObject();
                ...
                wordJs.put("library_id", word.getLibrary_id());
                wordJs.put("content", word.getContent());
                wordJs.put("skill_level", word.getSkill_level() + 1);

                wordList.put(wordJs);
            }
            req2.put("wordList", wordList);
            Rest2.sendRequest(Config.URL + "UpdateReviewWord.api", req2, new Callback() {
                @Override
                public void onFailure(Call call, IOException e) {
                    e.printStackTrace();
                }
